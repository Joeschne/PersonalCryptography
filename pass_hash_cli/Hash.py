...
def string_to_number_dynamic(card_info, base, min_length=991):
    ...
    # Use the dictionary to encode the string into a concatenated number
    raw_number = "".join(str(char_to_num[char]) for char in card_info)

    # Step 1: Scramble the raw number with a cascade modifier
    cascade = base
    scrambled = []
...
def number_to_string(num, ascii_min=32, ascii_max=126):
    ...
    # Initialize a dynamic scramble factor based on the input number
    scramble_factor = (num % ascii_range) + ascii_min
    cascade = 0  # A cascading variable to mix in previous iterations
...
def generate_prime(seed: int, limit: int = 10**6) -> int:
    # Dynamically generate a prime number starting from the given seed
    seed = int(seed)
    def is_prime(n):
        if n < 2:
            return False
    ...
    for num in range(seed + 50, limit):
        if is_prime(num):
            return num
    return 2  # Fallback (unlikely to happen with a reasonable seed/limit)
...
def calculate_primary_keys(card_info, card_number, base):
    ...
    ) % (base * 100)  # Scale to avoid clustering in a small range

    # Step 2: Generate a large prime for `key1mod`
    ...
    # Generate a large prime for `key2mod` to introduce unpredictability
    ...
    # Normalize `key2` to a large, dynamic range
    ...
    # - Uses a multiplicative hash to compute a base value
    ...
    # - Generate a dynamic prime number for `key3mod`
    ...
    # - Incorporate a right-shift for additional variability
    key3 = 0
    for i, c in enumerate(card_info):
...
def combine_keys_secure(key1, key2, key3):
    ...
    base = 31 + (total_length % 19)  # Add a dynamic component to base
    ...
    # Step 4: Normalize and fold into a large number
    # Generate a dynamic modulus with a controlled lower bound
    prime_mod = max(
        generate_prime(
            abs(key1 + key2 * key3 + cascade % (10 ** (total_length % 4 + 3)))
        ),
        997,
    )  # Ensure prime_mod is at least 997
    ...
    # Step 6: Final scaling and dynamic modulus with a larger range
...
def split_number_by_key(card_number, key1, key2, key3, base):
    ...
    for i in range(16):
        ...
        key1 = abs(key1) % (10**6)  # Keep the key within a reasonable range
        key2 = abs(key2) % (10**6)
        key3 = abs(key3) % (10**6)
...
def hash_input(card_info, base):
    ...
    for part in parts:
        key1, key2, key3 = calculate_primary_keys(number_to_string(part), card_number, base)
        output += charset_full[combine_keys_secure(key1, key2, key3) % len(charset_full)]
    return output

def hash(key, input, base=17):
    key_number = string_to_number_dynamic(key, generate_prime(base))
    key1, key2, key3 = calculate_primary_keys(key, key_number, generate_prime(base))
